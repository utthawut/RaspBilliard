...
{
	cout << "smallest root of cubic:" << solve_cubic(0.1, 0.2, 0.3, 0.4) << endl;
...
{
	long double coefs[5] = {coef_a, coef_b, coef_c, coef_d, coef_e};
    long double real_roots[4];
    long double imag_roots[4];


	QuadCubicRoots(coefs, 4, real_roots, imag_roots);
	return find_smallest(4, real_roots, imag_roots);
}
...
{
	long double coefs[4] = {coef_a, coef_b, coef_c, coef_d};
    long double real_roots[4];
    long double imag_roots[4];


	QuadCubicRoots(coefs, 3, real_roots, imag_roots);
	return find_smallest(3, real_roots, imag_roots);
}

long double solve_quadratic(const long double coef_a, const long double coef_b, const long double coef_c)
{
	long double coefs[3] = {coef_a, coef_b, coef_c};
    long double real_roots[4];
    long double imag_roots[4];


	QuadCubicRoots(coefs, 2, real_roots, imag_roots);
	return find_smallest(2, real_roots, imag_roots);
}

long double find_smallest(int n, long double *real_roots, long double *imag_roots)
{
	long double smallest_real;
	bool find_root = false;


	smallest_real = 10;     // more than 10 is impossible for event prediction on billard table
	for (int i = 0; i < n; i++)
	{
		if (imag_roots[i] == 0)
		{
			if ((real_roots[i] < smallest_real) && (real_roots[i] > 0))
			{
				smallest_real = real_roots[i];
				find_root = true;
			}
	    }
	}
    ...
    if (find_root)
    {
	    return smallest_real;
	}
	else
	{
	    return -31;     // cannot find root speed up python checking process
	}
